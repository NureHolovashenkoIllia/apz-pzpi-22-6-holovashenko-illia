...
public class BuildingController {
    ...
    @Operation(summary = "Get buildings of authenticated user", description = "Returns buildings for the authenticated user.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Buildings retrieved successfully",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = BuildingDto.class))),
            @ApiResponse(responseCode = "404", description = "No buildings found",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(example = "{\"error\":\"No buildings found for user\"}")))
    })
    @GetMapping("/user")
    @PreAuthorize("hasAnyAuthority('system_administrator', 'global_administrator', 'customer')")
    public ResponseEntity<?> getBuildingsForAuthenticatedUser(@AuthenticationPrincipal UserDetails userDetails) {
        try {
            return ResponseEntity.ok(buildingService.extractUserIdFromPrincipal(userDetails));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).
                    body("{\"Помилка при отримання будівель користувача\":\"" + e.getMessage() + "\"}");
        }
    }
